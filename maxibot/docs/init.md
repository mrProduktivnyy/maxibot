...
* **get_updates** (`offset`, `limit`, `timeout`, `allowed_updates`, `long_polling_timeout`) - Получает накопленные обновления одним запросом (GET /updates) и возвращает список Update — как в telebot. Свой цикл: `while True: bot.process_new_updates(bot.get_updates())`. Не смешивать с polling()/infinity_polling — там свой маркер  
    * **offset** - маркер MAX, а не update_id: маркер один на пачку и указывает на следующее обновление, поэтому арифметика не нужна. Телеботовское `offset=last_update_id + 1` распознаётся и исправляется с предупреждением (в том числе на первом вызове, когда маркера ещё нет), None и 0 — продолжить со своего маркера, отрицательный — телеботовский пропуск накопленного (запросы с timeout=0, в ответ пустой список)  
    * **limit** / **long_polling_timeout** - 1–1000 и 0–90 по спеке MAX; значения приводятся к целому и обрезаются с предупреждением, limit=0 не отправляется (как в telebot — работает серверный дефолт 100). Длительность long polling задаёт long_polling_timeout (как в telebot); timeout там про соединение и используется, только если long_polling_timeout=None. Отличие от telebot: 0 в MAX — «ответить сразу», а не «взять дефолт», поэтому цикл без своей паузы будет опрашивать API непрерывно (предупреждение в лог)  
    * Предупреждения get_updates и нормализации типов печатаются один раз на набор значений — в цикле лог не засоряется  
    * **allowed_updates** - те же телеботовские имена, что у polling; если после нормализации не осталось ни одного типа MAX, фильтра не будет — MAX пришлёт ВСЕ обновления (предупреждение в лог)  
    * **last_update_id** - маркер последнего ответа (в telebot тут id обновления). Поллинг ведёт свой маркер и это поле не трогает; у самого Update поля update_id нет — атрибут есть и всегда None  
    * Объекты (Message, CallbackQuery) строятся для всех полученных обновлений, как в telebot; поллинг, наоборот, строит их только при наличии обработчиков (Chat ходит в API за названием чата)  
* **process_new_updates** (`updates`) - Публичная точка входа: прогоняет список обновлений через весь пайплайн (middleware, затем обработчики). Для своего цикла get_updates и для кастомных webhook-интеграций (Flask, FastAPI, aiohttp вместо start_webhook): `bot.process_new_updates([request.get_json()])`. Принимает и словари MAX, и Update (сырой Update.de_json без api бот разберёт сам, заполнив тот же объект — но только если этот тип обновления есть кому отдать); непонятный элемент пропускается с предупреждением и не отменяет остальные, одиночное обновление вместо списка обрабатывается с предупреждением (забытые скобки — частая ошибка в вебхуке)  
* **process_new_messages** / **process_new_edited_messages** / **process_new_callback_query** (`список Message` / `список CallbackQuery`) - Публичные точки пайплайна, как в telebot. process_new_messages: сначала ожидающие register_next_step_handler, затем слушатели set_update_listener, затем message_handler; забранное next_step дальше не идёт. Переданный список, в отличие от telebot, не изменяется  
* **set_update_listener** (`listener`) - Добавляет слушателя новых сообщений (их может быть несколько); каждый получает список Message раньше обработчиков. Отличия от telebot: список всегда из одного сообщения (maxibot обрабатывает обновления по одному), а при threaded=True слушатель лишь ставится в очередь раньше обработчика. Посты каналов, правки, коллбэки и события членства слушателям не приходят — как в telebot  
...
* **channel_post_handler** / **edited_channel_post_handler** (`commands`, `regexp`, `func`, `content_types`) - Декораторы обработчиков постов каналов и их правок. Отдельного типа обновления для каналов в MAX нет: пост приходит как message_created/message_edited с chat_type='channel', и maxibot, как telebot, разводит сам — посты каналов попадают ТОЛЬКО в канальные обработчики (не в message_handler/edited_message_handler и не в next_step). У поста от имени канала message.from_user = None (как в telebot). В middleware посты лежат в update.message/update.edited_message; телеботовские поля update.channel_post/edited_channel_post существуют и всегда None  
* **add_channel_post_handler** / **add_edited_channel_post_handler** (`handler_dict`), **register_channel_post_handler** / **register_edited_channel_post_handler** (`callback`, `content_types`, `commands`, `regexp`, `func`, `pass_bot`) - Недекораторная регистрация канальных обработчиков; без content_types register_ матчит посты любого типа (как в telebot)  
* **process_new_channel_posts** / **process_new_edited_channel_posts** (`список Message`) - Публичные точки пайплайна: прогоняют посты/правки постов по канальным обработчикам (как в telebot)  
* **my_chat_member_handler** (`func`) - Декоратор обработчика изменений статуса САМОГО БОТА (ChatMemberUpdated): bot_added/bot_removed (left↔member), bot_started (kicked→member, аналог разблокировки; /start-сообщение тоже диспатчится), bot_stopped (member→kicked — аналог блокировки бота, главный сценарий my_chat_member в telebot). Фильтр только func, как в telebot  
* **chat_member_handler** (`func`) - Декоратор обработчика статуса ДРУГИХ участников: user_added/user_removed (left↔member; MAX шлёт их только там, где бот админ). В отличие от Telegram видны только вход/выход (промоуты событий не порождают); в telebot тип требует allowed_updates — в MAX приходит сам. from_user — кто добавил/удалил (по ссылке/сам вышел — сам пользователь)  
* **add_my_chat_member_handler** / **add_chat_member_handler** (`handler_dict`), **register_my_chat_member_handler** / **register_chat_member_handler** (`callback`, `func`, `pass_bot`) - Недекораторная регистрация обработчиков членства  
    * ChatMemberUpdated строится только при подписке: обработчик членства; middleware `update_types=['my_chat_member']`/`['chat_member']` (получает сам объект, как в telebot); общий middleware (получает Update целиком); middleware на сырые bot_removed/bot_stopped/user_added/user_removed — им тоже приходит Update с объектом. Подписка middleware на сырые bot_added/bot_started даёт Message: эти события обрабатываются и как сообщения  
    * Порядок: событие статуса ставится в очередь перед /start-сообщением; при threaded=True фактический порядок выполнения не гарантирован (обработчики уходят в пул). Упавший middleware — как и везде — отменяет обработку всего обновления, включая сообщение  
    * `allowed_updates` у polling/start принимает и телеботовские имена: 'my_chat_member' разворачивается в bot_added/bot_removed/bot_started/bot_stopped, 'chat_member' — в user_added/user_removed, 'message' — в message_created; несуществующие в MAX типы убираются с предупреждением  
    * Дубли bot_added от MAX (одно добавление приходит дважды) отсеиваются в самом боте — одинаково для поллинга и webhook; повторное добавление в тот же чат позже проходит (ключ учитывает время события)  
* **process_new_my_chat_member** / **process_new_chat_member** (`список ChatMemberUpdated`) - Публичные точки пайплайна событий членства (как в telebot)  
...
* **_process_text_message** (`context`) - Обрабатывает входящее сообщение  
    * **context** - Объект типа `Message`  
* **_process_update** (`update`, `parsed`) - Метод для обработки входящего полученного обновления  
    * **update** - Словарь, полученный от API MAX в процессе опроса API на обновления  
    * **parsed** - Готовый Update этого же обновления (его передаёт process_new_updates), чтобы не разбирать payload второй раз; сырой Update из de_json без api достраивается здесь же — после проверки, что обработчики вообще есть  
